...
def _build_rules_text(rule_path):
    """Read rules file as enumerated text."""
...
def build_shieldlm_prompt(query, response, lang, model_base, rules_text=None):
    """Construct ShieldLM judge prompt with optional rules injection."""
    if lang == 'zh':
...
if __name__ == "__main__":
    ...
    # ShieldLM-style experiment parameters
    parser.add_argument(
        '--input_jsonl', default=None, type=str,
        help='Path to input JSONL with fields: query, response. If set, overrides --predict_data.'
    )
    parser.add_argument(
        '--output_jsonl', default=None, type=str,
        help='Path to write merged JSONL with added field "output".'
    )
    parser.add_argument(
        '--lang', default=None, type=str, choices=('en', 'zh'),
        help='Language for ShieldLM prompt construction.'
    )
    parser.add_argument(
        '--model_base', default=None, type=str, choices=('qwen', 'baichuan', 'internlm', 'chatglm'),
        help='Base model family to select minimal chat template.'
    )
    parser.add_argument(
        '--rule_path', default=None, type=str,
        help='Optional rule file (.txt), injected into the analysis instruction.'
    )
    ...
    if config_.run_mode == 'predict':
        # Priority: input_jsonl -> predict_data
        if args_.input_jsonl is not None:
            if args_.lang is None or args_.model_base is None:
                raise ValueError("When using --input_jsonl, please set both --lang and --model_base.")
            rules_text = _build_rules_text(args_.rule_path) if args_.rule_path else None
            input_texts = []
            with open(args_.input_jsonl, "r", encoding="utf-8") as f_jsonl:
                for line in f_jsonl:
                    if not line.strip():
                        continue
                    obj = json.loads(line)
                    ...
                    input_texts.append(
                        build_shieldlm_prompt(query, response, args_.lang, args_.model_base, rules_text)
                    )
            config_.input_data = input_texts
        else:
    ...
    # Post-process: merge outputs back to JSONL if requested
    if config_.run_mode == 'predict' and args_.input_jsonl is not None and args_.output_jsonl is not None:
        candidates = []
        if getattr(config_, "output_dir", None):
            candidates.append(os.path.join(config_.output_dir, "text_generation_result.txt"))
        candidates.append(os.path.join(os.getcwd(), "text_generation_result.txt"))
        result_path = None
        ...
        if result_path is None:
            logger.warning("text_generation_result.txt not found, skip JSONL merge.")
        else:
            outputs = []
            try:
                ...
                try:
                    ...
                    parsed = None
                if isinstance(parsed, dict) and "text_generation_text" in parsed:
                    outputs = parsed["text_generation_text"]
                elif isinstance(parsed, list):
                    outputs = parsed
                else:
                    outputs = [line.rstrip("\n") for line in content.splitlines() if line.strip() != ""]
            except Exception as e:
                logger.warning(f"Failed to read predict result file {result_path}: {e}")

            try:
                with open(args_.input_jsonl, "r", encoding="utf-8") as f_in, \
                        open(args_.output_jsonl, "w", encoding="utf-8") as f_out:
                    idx = 0
                    for line in f_in:
                        if not line.strip():
                            continue
                        obj = json.loads(line)
                        obj["output"] = outputs[idx] if idx < len(outputs) else None
                        f_out.write(json.dumps(obj, ensure_ascii=False) + "\n")
                        idx += 1
                logger.info(f"Merged predict outputs into {args_.output_jsonl}")
            except Exception as e:
                logger.warning(f"Failed to merge outputs to JSONL: {e}")
